...
export const BASE_URL = process.env.REACT_APP_API_URL;
 const TicketBySource = () => {
  ...
  const getTotalTickets= async (range,date_1,date_2) => {
    var date='';
    if(range==false){
         date=`/countBySource?date=${date_1}`;
    }else{
         date=`/countBySource?startDate=${date_1}&endDate=${date_2}`
    }
    // console.log(date)
    try {
...
const [ticketModal, setTicketModal] = useState(false);
  ...
  const option = {
    ...
    yAxis: {
      type: 'category',
      data: tickets.map((item) => item.source_name), // Map the source names to yAxis data
    },
    series: [
      {
        ...
        type: 'bar',
        label: {
          show: false, // Set show to false to hide the labels
        },
     
        data: tickets.map((item, index) => ({
          value: Number(item.count),
          itemStyle: {
            color: ['orange', 'red', 'pink', 'green','purple','maroon'][index % 10],
          },
        })),
      },
    ],
...
<div style={{display:'flex',flexDirection:'row',width:'100%',justifyContent:'space-between'}}>
 ...
 <label style={{margin:0}} htmlFor="c77" >
 <input
          type="checkbox"
          id="c77"
          checked={isChecked1}
